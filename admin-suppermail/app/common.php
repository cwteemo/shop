...
// 平台字段 key
define("KEY_PLATFORM", '_platform');
// admin 平台
define("PLATFORM_ADMIN", "ADMIN");
// app 平台
define("PLATFORM_APP", "APP");
// 微信小程序
define("PLATFORM_WX_APP", "WX_APP");
 ...
 */
function secho($str, string $tag='')
{
    $array = debug_backtrace()[0];
    $file = str_replace(root_path(), '', $array['file']);
    ...
    $args = implode('</b><i>,</i> <b>', $array['args']);

    $str = colorize("<i>$file [{$array['line']}]</i> {$array['function']}(<b>{$args}</b>)");
...
{
    $isBrowser = !request()->isCli();

    if ($isBrowser) {
        $map = [ 'e' => '#F56C6C', 's' => '#67C23A', 'w' => '#E6A23C', 'i' => '#909399', 'b' => '#409EFF' ];
    } else {
        $map = [
            // error
            'e' => '[31m',
    ...
    //     $map['b'] = "[{$i}m";
        foreach ($map as $key => $val) {
            if ($isBrowser) {
                $replace = ["<span style='color:$val;'>", '</span>'];
            } else {
                $replace = [chr(27).$val, chr(27)."[0m"];
            }
            $text = str_replace(["<$key>", "</$key>"],  $replace, $text);
        }
        // echo "$i => ".str_replace(["<$key>", "</$key>"],  $replace, $text).PHP_EOL;
    // }
 ...
 * @param $order (默认是升序)
 */
function sortTime(&$arr, $order='ASC')
{
    usort($arr, function ($time1, $time2) use ($order) {
        $time1 = strtotime($time1);
        $time2 = strtotime($time2);
        if ($time1 == $time2) {
            return 0;
        }
        $value = ($time1 - $time2) / abs($time1 - $time2);
 ...
 * @return string
 */
function getTime($time_format='Y-m-d H:i:s.u')
{
    return \DateTime::createFromFormat('0.u00 U', microtime())->setTimezone(new \DateTimeZone(date_default_timezone_get()))->format($time_format);
}
 ...
 * @return array
 */
function group_visit_date($visit, $key='create_time')
{
    $curyear = date('Y');
    $visit_list = [];
    foreach ($visit as $v) {
        $time = strtotime($v[$key]);
        if ($curyear == date('Y', $time)) {
            $date = date('n月j日', $time);
        } else {
            $date = date('Y年n月j日', $time);
        }
        $v['time'] = date("G:i", $time);
        $visit_list[$date][] = $v;
    }
    return $visit_list;
}
 ...
 * @return array
 */
function group_visit($visit, $key, $type_key=null)
{
    $visit_list = [];
    foreach ($visit as $v) {
        if (!$type_key || ($type_key && $v[$type_key])) {
            $val = $v[$key];
            $visit_list[$val][] = $v;
        }
    }
...
{
    $error = \think\facade\Db::transaction($callback);
    if ($error) {
        \app\lib\Res::returnErr($error->message);
    }
}
